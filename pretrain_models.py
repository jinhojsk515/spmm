...
class biALBEF(pl.LightningModule):
    def __init__(self, tokenizer=None, config=None, loader_len=0):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        arg_sche = utils.AttrDict(self.config['schedular'])
    ...
    def training_epoch_end(self, outputs):    # outputs: collection of returns from 'training_step'
        tmp = torch.stack([o['loss'] for o in outputs[-10:]]).mean(dim=0).tolist()
        if self.global_rank == 0:
